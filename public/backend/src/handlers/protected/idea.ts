...
export const deleteIdeaById = async (id: any) => {
  try {
    const collections = await database_connection([
      "Idea",
      "Faq",
    ...
    const replyCollection = collections[4];
    const results = await Promise.all([
      ideaCollection.deleteOne({ _id: new ObjectId(id) }),
      faqCollection.deleteMany({ ideaId: id }),
      updateCollection.deleteMany({ ideaId: id }),
      commentCollection.deleteMany({ ideaId: id }),
      replyCollection.deleteMany({ ideaId: id }),
    ]);
    if (!results) {
      return false;
    }
