...
export const getAllIdeas = async (req, res) => {
  try {
    ...
    const ideas = await ideaCollection
      .aggregate([{ $sort: { date: -1 } }, { $skip: skip }, { $limit: limit }])
      .toArray();
    totalIdeas = await ideaCollection[0].countDocuments();
    res.json({ ideas, totalIdeas, message: "ok" }).status(200).end();
  } catch (error) {
...
export const getOneIdea = async (req, res) => {
    ...
    try {
        const collections = await database_connection(["Idea", "User"]);
        const ideaCollection = collections[0];
        const idea = await ideaCollection.findOne({ ideaId: req.params.ideaid });
        const userCollection = collections[1];
        const user = await userCollection.findOne({ userId: idea.userId });
...
export const getIdeafaqs = async (req, res) => {
    ...
    try {
        ...
        const faqs = await faqCollection.find({ ideaId: req.params.ideaid }).toArray();
...
try {
    ...
    const updates = await updateCollection.find({ ideaId: req.params.ideaid }).toArray();
...
export const getIdeaByCategory = async (req, res) => {
    ...
    try {
        const collections = await database_connection(["Idea"]);
        const ideaCollection = collections[0];
        const ideas = await ideaCollection.find({ categoryId: req.params.id }).toArray();
...
export const getCommentsAndReplies = async (req, res) => {
    ...
    try {
        ...
        const comments = await commentCollection.aggregate([{ $match: { ideaId: req.params.id } }, { $sort: { commentedAt: -1 } }, { $skip: skip }, { $limit: limit }]).toArray();
        const replyCollection = collections[1];
        comments.forEach(async (comment) => {
            comment.replies = await replyCollection.find({ commentId: comment.commentId }).toArray();
        });

        res.json({ comments, message: "ok" }).status(200).end();

    } catch (error) {
        console.error("Error occurred while getting comments and replies...\n", error);
    }
...
export const getIdeaByUserId = async (req, res) => {
    ...
    try {
        const collections = await database_connection(["Idea"]);
        const ideaCollection = collections[0];
        const ideas = await ideaCollection.find({ userId: req.params.userid }).toArray();
        res.json({ ideas, message: "ok" }).status(200).end();
    } catch (error) {
